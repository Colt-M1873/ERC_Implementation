...
| 作者           | [William Entriken](https://github.com/fulldecent), [Dieter Shirley](mailto:dete@axiomzen.co), [Jacob Evans](mailto:jacob@dekz.net), [Nastassia Sachs](mailto:nastassia.sachs@protonmail.com) |
| -------------- | ------------------------------------------------------------ |
...
- [动机](#动机)
- [规范](#规范)
  - [Caveats](https://eips.ethereum.org/EIPS/eip-721#caveats)
- [Rationale](https://eips.ethereum.org/EIPS/eip-721#rationale)
- [Backwards Compatibility](#backwards-compatibility)
- [Test Cases](https://eips.ethereum.org/EIPS/eip-721#test-cases)
- [Implementations](https://eips.ethereum.org/EIPS/eip-721#implementations)
- [References](https://eips.ethereum.org/EIPS/eip-721#references)
- [Copyright](https://eips.ethereum.org/EIPS/eip-721#copyright)
...
以下标准涉及在智能合约中为 NFT 实施标准 API。 该标准提供了跟踪和传输 NFT 的基本功能。

We considered use cases of NFTs being owned and transacted by individuals as well as consignment to third party brokers/wallets/auctioneers (“operators”). NFTs can represent ownership over digital or physical assets. We considered a diverse universe of assets, and we know you will dream up many more:

我们考虑了 NFT 由个人拥有和交易，以及委托给第三方经纪人/钱包/拍卖商（“运营商”）的用例。 NFT 可以代表对数字或实物资产的所有权。 我们考虑了各种各样的资产，并知道您会有更多的想法：
...
一般来说，每一栋房子都是独特的，也没有哪两只小猫是完全一样的。 NFT 是*可区分（非同质化）*的，您必须分别跟踪每个 NFT 的所有权。
...
标准接口允许钱包/经纪人/拍卖应用程序与以太坊上的任何 NFT 一起工作。 我们提供简单的 ERC-721 智能合约，也提供了跟踪*无限多* NFT 的合约。 对于其他应用的讨论见下文。
...
该标准受到 ERC-20 代币标准的启发，并建立在 EIP-20 创建以来两年的经验之上。 EIP-20 不足以跟踪 NFT，因为NFT定义的每个资产都是不同的（非同质化的），而EIP-20所定义的某种代币的每一个都是相同的（同质化的）。

Differences between this standard and EIP-20 are examined below.

此标准与 EIP-20 之间的差异如下所述。

##  规范

The key words “MUST”, “MUST NOT”, “REQUIRED”, “SHALL”, “SHALL NOT”, “SHOULD”, “SHOULD NOT”, “RECOMMENDED”, “MAY”, and “OPTIONAL” in this document are to be interpreted as described in RFC 2119.

本文档中的关键词【必须】（MUST），【不得】（MUST NOT），【必要的】（REQUIRED），【要求】（SHALL），【要求不】（SHALL NOT），【应当】（SHOULD），【不应】（SHOULD NOT），【推荐的】（RECOMMENDED），【不推荐的】（NOT RECOMMENDED），【可以】（MAY），和【可选的】（OPTIONAL）按照[RFC2119](https://www.rfc-editor.org/rfc/rfc2119)中的说明进行解释。

> 译者注：[RFC2119原文链接](https://datatracker.ietf.org/doc/html/rfc2119)，RFC英文原版中的关键词以全部大写来表示，例如：MUST。而中文没有大小写之分，古早的RFC中译版大多没有对关键词和非关键词进行区分处理，难以还原原文的严谨性，故在本文中对关键词使用加中文方括号的方式进行处理，例如：【必须】（MUST）。关于RFC2119的关键词中译的讨论，见[此处](另写一篇文章单独将中译的可能规范讲一下。目前RFC翻译标准还不够统一，也没有查到官方给出的中译规范。因而存在着一些问题，首先是关键词的区分，因为英文中关键词以全部大写来表示，而中文没有大小写之分，故很多RFC中译版没有对关键词和非关键词进行区分处理，未能还原出英文原文中关键词全部大写，非关键词小写的区别。如果不对每个关键词都进行特意区分的话，特别是以机翻为基础的译者，很容易出现语义上的错误。例如：SHALL和SHOULD这两个词很容易被统一翻译成“应当”，但在RFC2119的规定中，SHALL和SHOULD的强调级别是不同的，SHALL和MUST同级，表示绝对的毫无例外情况的；而SHOULD和RECOMMENDED同级，表示大多数时候成立，但存在着某些特殊情况。SHALL和SHOULD的适用场景是不同的，译文中将二者混用会造成语义的混乱，损害严谨性，对实践应用产生不利影响。因此，中译要将关键词做到中英词汇一一对应，并将关键词和非关键词进行区分标注，以最大程度维持严谨性，降低读者的理解成本)，
